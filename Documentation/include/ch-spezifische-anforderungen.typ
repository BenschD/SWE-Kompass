...
Das folgende Use-Case-Diagramm ordnet die Anwendungsfälle der öffentlichen API den Akteuren zu. Primärakteur ist die Host-Anwendung; die Sekundärakteure (What3Words-API, Dateisystem, SLF4J-Backend) werden vom System genutzt. Die normative Detailspezifikation je Anwendungsfall folgt anschließend in den `/LF…/`-Karten.

#mermaid-figure(
  [Use-Case-Diagramm: Anwendungsfälle der öffentlichen API mit Primär- und Sekundärakteuren.],
  "use_case",
  width: 68%,
)
...
#ll-card("/LL040/", "Build-Reproduzierbarkeit", "Projektauftrag", "-",
  [`mvn clean test` auf frisch geklontem Repository erfolgreich und Abhängigkeiten über Maven Central.])

#ll-card("/LL050/", "XML-Escaping", "Sicherheitsrichtlinie", "/LF140/",
  [Nutzerstrings in GPX werden escaped (`XmlEscaper`) und Nachweis über /TC100/ und GPX-Writer-Tests.])
...
#ll-card("/LL070/", "Test-Determinismus", "Testbarkeitsanforderung", "-",
  [Zeitabhängige Tests nutzen `ClockPort`-Mocks und kein `Thread.sleep()` im Testcode.])
...
#figure(
  ...
  align(left, table(
    ...
    [`IllegalArgumentException`], [
      ...
      ungültige `GeoCoordinate` bei Erzeugung und\
...
Verbindliche Testfälle `/TC010/` … `/TC150/` mit JUnit-Methodennamen (siehe `implementation/TRACEABILITY.md`). Ergänzende Nachweise ohne `/TC…/` (z. B. `/LF150/`) stehen dort im gleichen Dokument.
...
#mermaid-figure(
  [Kontrollflussgraph von `TrackAggregator.accept` (Whitebox-Sicht, v(G) = 6).],
  "kontrollflussgraph",
  width: 100%,
  height: 14cm,
)
